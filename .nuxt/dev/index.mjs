...
const _R3F8dNMtwM22UUCN9tYVfessTmHi2Wkk6hRdFVjaVDc = defineNitroPlugin(async () => {
  ...
  try {
    ...
    console.error("Error en migraci\xF3n de Sprints:", err);
  }
});
...
class AuthService {
  static async register(data) {
    const existingUser = await db.selectFrom("users").select("id").where("email", "=", data.email).executeTakeFirst();
    if (existingUser) {
      throw new Error("El correo ya est\xE1 en uso");
    }
    ...
    }).returning(["id", "email", "name", "avatar_url", "created_at"]).executeTakeFirstOrThrow();
    const token = this.generateToken(newUser.id, newUser.email);
    return { user: newUser, token };
  }
  static async login(email, password_raw) {
...
const login_post = defineEventHandler(async (event) => {
  try {
    ...
    if (!body.email || !body.password) {
      throw createError({ statusCode: 400, statusMessage: "Email y contrase\xF1a requeridos" });
    }
    const { user, token } = await AuthService.login(body.email, body.password);
    setCookie(event, "auth_token", token, {
      httpOnly: false,
      // Permitimos que el cliente lo lea para la hidratación en este caso
      maxAge: 60 * 60 * 24 * 7,
      // 7 días
      path: "/"
    });
...
class IssueService {
  static async getIssuesByProject(projectId) {
    return await db.selectFrom("issues").selectAll().where("project_id", "=", projectId).orderBy("position", "asc").execute();
  }
  static async createIssue(data) {
    return await db.transaction().execute(async (trx) => {
      const lastIssue = await trx.selectFrom("issues").select("key_number").where("project_id", "=", data.project_id).orderBy("key_number", "desc").limit(1).executeTakeFirst();
      const nextKeyNumber = lastIssue ? lastIssue.key_number + 1 : 1;
      const lastInColumn = data.column_id ? await trx.selectFrom("issues").select("position").where("column_id", "=", data.column_id).orderBy("position", "desc").limit(1).executeTakeFirst() : null;
      const nextPosition = lastInColumn ? lastInColumn.position + 1024 : 1024;
      return await trx.insertInto("issues").values({
...
const _id__patch$2 = defineEventHandler(async (event) => {
  ...
  if (Object.keys(updates).length === 0) {
    throw createError({ statusCode: 400, statusMessage: "No se enviaron campos v\xE1lidos para actualizar" });
  }
  try {
...
const _id__put$2 = defineEventHandler(async (event) => {
  ...
  if (body.action === "move") {
    if (!body.toColumnId || body.newPosition === void 0) {
      throw createError({ statusCode: 400, statusMessage: "Faltan par\xE1metros para mover (toColumnId, newPosition)" });
    }
...
const comments = defineEventHandler(async (event) => {
  ...
  if (method === "GET") {
    const comments = await db.selectFrom("issue_comments").innerJoin("users", "users.id", "issue_comments.user_id").select([
      "issue_comments.id",
    ...
    ]).where("issue_id", "=", issueId).orderBy("created_at", "asc").execute();
    return comments;
  }
  if (method === "POST") {
    const body = await readBody(event);
    ...
    const newComment = await db.insertInto("issue_comments").values({
      issue_id: issueId,
      user_id: senderId,
    ...
    return {
      ...
      user_id: senderId,
      user_name: user == null ? void 0 : user.name,
      user_avatar: user == null ? void 0 : user.avatar_url
    };
  }
});
...
const time_get = defineEventHandler(async (event) => {
  ...
  const logs = await db.selectFrom("time_logs").selectAll().where("issue_id", "=", issueId).orderBy("logged_at", "desc").execute();
  return logs;
});

const time_get$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: time_get
}, Symbol.toStringTag, { value: 'Module' }));

const time_post = defineEventHandler(async (event) => {
  const issueId = getRouterParam(event, "id");
  const body = await readBody(event);
  if (!issueId) {
    throw createError({ statusCode: 400, statusMessage: "ID de incidencia es requerido" });
...
const time_post$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: time_post
}, Symbol.toStringTag, { value: 'Module' }));
...
const notifications = defineEventHandler(async (event) => {
  ...
  if (method === "GET") {
    const notifs = await db.selectFrom("notifications").leftJoin("users", "users.id", "notifications.sender_id").leftJoin("issues", "issues.id", "notifications.issue_id").select([
      "notifications.id",
      "notifications.type",
    ...
    ]).where("notifications.user_id", "=", currentUserId).orderBy("notifications.created_at", "desc").execute();
    return notifs;
  }
  if (method === "PATCH") {
    const body = await readBody(event);
    if (body.action === "mark_all_read") {
      await db.updateTable("notifications").set({ read_at: /* @__PURE__ */ new Date() }).where("user_id", "=", currentUserId).where("read_at", "is", null).execute();
      return { success: true };
    }
    if (body.notification_id) {
      await db.updateTable("notifications").set({ read_at: /* @__PURE__ */ new Date() }).where("id", "=", body.notification_id).where("user_id", "=", currentUserId).execute();
      return { success: true };
    }
    throw createError({ statusCode: 400, statusMessage: "Acci\xF3n no v\xE1lida" });
  }
});
...
class ProjectService {
  static async getAllProjects() {
    return await db.selectFrom("projects").selectAll().orderBy("created_at", "desc").execute();
  }
  static async getProjectById(id) {
    return await db.selectFrom("projects").selectAll().where("id", "=", id).executeTakeFirst();
  }
  static async createProject(data) {
    return await db.insertInto("projects").values({
  ...
  static async getProjectColumns(projectId) {
    ...
    if (columns.length === 0) {
      const defaultColumns = [
        { project_id: projectId, name: "Por Hacer", position: 1e3 },
        { project_id: projectId, name: "En Progreso", position: 2e3 },
        { project_id: projectId, name: "Listo", position: 3e3 }
      ];
      const inserted = await db.insertInto("board_columns").values(defaultColumns).returningAll().execute();
...
const columns = defineEventHandler(async (event) => {
  const method = event.node.req.method;
...
const features_patch = defineEventHandler(async (event) => {
  const projectId = getRouterParam(event, "id");
  if (!projectId) {
  ...
  const user = await db.selectFrom("users").select("is_master_admin").where("id", "=", userId).executeTakeFirst();
  if (!user || !user.is_master_admin) {
    throw createError({ statusCode: 403, statusMessage: "Acceso denegado: Se requiere rol de SuperAdmin" });
  }
  const body = await readBody(event);
  const updatedProject = await db.updateTable("projects").set({ enabled_features: JSON.stringify(body) }).where("id", "=", projectId).returningAll().executeTakeFirst();
  if (!updatedProject) {
    throw createError({ statusCode: 404, statusMessage: "Proyecto no encontrado" });
  }
...
const features_patch$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: features_patch
}, Symbol.toStringTag, { value: 'Module' }));
...
const burndown = defineEventHandler(async (event) => {
  ...
  const issues = await db.selectFrom("issues").selectAll().where("sprint_id", "=", sprintId).execute();
  const start = sprint.start_date ? new Date(sprint.start_date) : /* @__PURE__ */ new Date();
  const end = sprint.end_date ? new Date(sprint.end_date) : new Date(start.getTime() + 7 * 24 * 60 * 60 * 1e3);
  const totalDays = Math.max(1, Math.ceil((end.getTime() - start.getTime()) / (1e3 * 3600 * 24)));
  const totalIssues = issues.length;
  const labels = [];
  const idealData = [];
  const realData = [];
  for (let i = 0; i <= totalDays; i++) {
    const currentDay = new Date(start.getTime() + i * 24 * 60 * 60 * 1e3);
    labels.push(`D\xEDa ${i}`);
    ...
    const closedOnOrBefore = issues.filter((issue) => {
      return new Date(issue.updated_at) <= currentDay && issue.column_id;
    }).length;
    realData.push(totalIssues - closedOnOrBefore);
  }
  const resolvedIssues = issues.filter((i) => new Date(i.updated_at) < /* @__PURE__ */ new Date());
  ...
  return {
    labels,
    series: [
      { name: "L\xEDnea Ideal", data: idealData },
    ...
    kpis: {
      ...
      avgEstimatedTime: "4.5h"
      // Mock, ya que no tenemos campo de time tracking en la bd actual
    }
  };
});

const burndown$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: burndown
}, Symbol.toStringTag, { value: 'Module' }));
...
class SprintService {
  static async getProjectSprints(projectId) {
    return await db.selectFrom("sprints").selectAll().where("project_id", "=", projectId).orderBy("start_date", "asc").execute();
  }
  static async createSprint(data) {
    return await db.insertInto("sprints").values({
      project_id: data.project_id,
      name: data.name,
      goal: data.goal || null,
  ...
  static async updateSprint(sprintId, updates) {
    if (Object.keys(updates).length === 0) return null;
    return await db.updateTable("sprints").set(updates).where("id", "=", sprintId).returningAll().executeTakeFirst();
  }
}
...
const team_get = defineEventHandler(async (event) => {
  ...
  const members = await db.selectFrom("project_members").innerJoin("users", "users.id", "project_members.user_id").select([
    "users.id",
    "users.name",
    "users.email",
    "users.avatar_url",
    "project_members.role"
  ]).where("project_members.project_id", "=", projectId).execute();
  const workloadPromises = members.map(async (member) => {
    const issues = await db.selectFrom("issues").select(["id", "estimated_minutes"]).where("project_id", "=", projectId).where("assignee_id", "=", member.id).execute();
    const issueCount = issues.length;
    const totalEstimated = issues.reduce((sum, issue) => sum + (issue.estimated_minutes || 0), 0);
    const issueIds = issues.map((i) => i.id);
    let totalLogged = 0;
    if (issueIds.length > 0) {
      const logs = await db.selectFrom("time_logs").select(["duration_minutes"]).where("issue_id", "in", issueIds).execute();
...
const _id__patch = defineEventHandler(async (event) => {
  ...
  if (Object.keys(updates).length === 0) {
    throw createError({ statusCode: 400, statusMessage: "No hay campos v\xE1lidos para actualizar" });
  }
  try {
...
const github_post = defineEventHandler(async (event) => {
  try {
    ...
    if (payload.commits && Array.isArray(payload.commits)) {
      textToParse = payload.commits.map((c) => c.message).join(" ");
...
const github_post$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: github_post
}, Symbol.toStringTag, { value: 'Module' }));
